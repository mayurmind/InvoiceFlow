...
describe('Audit Integration - GET /api/v1/audit', () => {
  ...
  it('returns 403 if authenticated but not SUPER_ADMIN', async () => {
    const res = await request(app).get('/api/v1/audit').set('x-mock-role', UserRole.STAFF);
    expect(res.status).toBe(403);
  });

  it('returns 200 with audit logs for SUPER_ADMIN', async () => {
    const res = await request(app).get('/api/v1/audit').set('x-mock-role', UserRole.SUPER_ADMIN);
